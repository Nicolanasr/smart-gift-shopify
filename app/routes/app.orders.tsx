                                ...
                                <tbody>
                                    {giftOrders.map((order: any, i: number) => {
                                        ...
                                        return giftItems.map((item: any, index: any) => {
                                            ...
                                            return (
                                                <tr key={`${order.id}-${item.id}`} style={{ background: i % 2 === 0 ? 'white' : '#fff' }}>
                                                    {index === 0 && (
                                                        <>
                                                            <td rowSpan={giftItems.length} style={styles.td}>
                                                                <a href={`shopify:admin/orders/${order.id.split('/').pop()}`} style={styles.link}>{order.name}</a>
                                                            </td>
                                                            <td rowSpan={giftItems.length} style={styles.td}>
